...
def main():
    ...
    model4 = resnet.ClassifierBaseModel().to(device)
    model4.load_state_dict(torch.load(config.Model_lesion_CE_weight_path + 'xxxxx.pth', map_location=device), strict=False)

    # 进行测试
    labels, predictions, lesion = test_model(model=model4, test_loader=test_loader)
    with open(config.Model_lesion_CE_Prediction_path + 'Test_Predictions_without.txt', 'w') as f:
        for loss in predictions:
            f.write(f"{loss}\n")

    # 绘制 ROC 曲线和计算 AUC
    average_list = np.array(predictions)
    ci(labels, average_list, config.Model_lesion_CE_ROC_path)

    # 绘制混淆矩阵
    threshold = 0.5
    binary_output = [1 if prob > threshold else 0 for prob in average_list]
    cm = confusion_matrix(labels, binary_output, labels=[1, 0])
    plot_confusion_matrix(cm, config.Model_lesion_CE_Matrix_path, classes=["Malignant", "Benign"])
    ...
    with open(config.Model_lesion_CE_Prediction_path + 'ImagePath_Label_Prediction_without.txt', 'w') as f:
        for line in result:
            f.write(line + '\n')
